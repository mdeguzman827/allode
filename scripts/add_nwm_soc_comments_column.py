"""
Migration script to add nwm_soc_comments column to properties table.
...
def migrate():
    ...
    col_name = "nwm_soc_comments"

    print("=" * 60)
    print("Add nwm_soc_comments to properties")
    print("=" * 60)
    ...
    if database_url.startswith("sqlite:///"):
        ...
        if col_name in existing:
            ...
            return True
        try:
            cursor.execute(f"ALTER TABLE properties ADD COLUMN {col_name} TEXT")
            conn.commit()
    ...
    try:
        with engine.connect() as conn:
            ...
            if col_name in existing:
                print(f"  ⊘ Column already exists: {col_name}")
                return True
            conn.execute(text(f"ALTER TABLE properties ADD COLUMN {col_name} TEXT"))
            conn.commit()
            print(f"  ✓ Added column: {col_name}")
        print("\n✓ Migration complete!")
        return True

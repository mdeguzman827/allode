...
class Property(Base):
    ...
    bathrooms_total_integer = Column(Float, index=True)  # Float to allow 1 decimal (e.g. 1.75)
    ...
    association_yn = Column(Boolean)
    buyer_brokerage_compensation = Column(String)
    buyer_brokerage_compensation_type = Column(String)
    cooling_yn = Column(Boolean)
    cooling = Column(Text)
    heating_yn = Column(Boolean)
    heating = Column(Text)
    internet_entire_listing_display_yn = Column(Boolean)
    internet_automated_valuation_display_yn = Column(Boolean)
    status_change_timestamp = Column(DateTime)
